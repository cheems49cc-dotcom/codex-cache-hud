...
internal sealed class BrushFactory
{
        ...
        bool hasAttention)
    {
        ...
        if (settings.TransparencyMode == "background" && role == BrushRole.Background && brush is SolidColorBrush backgroundSolid)
        {
            var backgroundColor = backgroundSolid.Color;
            backgroundColor.A = (byte)Math.Round(byte.MaxValue * Math.Clamp(settings.Opacity, 0, 1));
    ...
    public Brush CreateSurface(HudSettings settings, string status, bool hasAttention)
    {
        ...
        if (settings.ThemeStyle.Surface == "image" && File.Exists(settings.ThemeStyle.BackgroundImage))
        {
            ...
            var imageBrush = new ImageBrush(source)
            {
                ...
                Opacity = settings.TransparencyMode switch
                {
                    "uniform" => 1,
                    "background" => settings.Opacity,
                    _ => settings.ThemeStyle.ImageOpacity
                }
            };
            imageBrush.Freeze();
            result = imageBrush;
        ...
        else if (settings.ThemeStyle.Surface == "gradient")
        {
            ...
            if (settings.TransparencyMode == "background")
            {
        ...
        bool hasAttention)
    {
        ...
        if (settings.TransparencyMode == "background")
        {
            return role == BrushRole.Background ? level : 1;
        }
