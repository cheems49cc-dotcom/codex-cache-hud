...
internal sealed partial class HudApplicationController : IDisposable
{
    ...
    public void Start()
    {
        _log.Write($"Codex Cache HUD v3.4.0 starting. config={_paths.ConfigPath}; profiles={string.Join(',', _profiles.Select(static profile => profile.Id))}; agentNotices={_settings.AgentNotifications.Enabled}/{_settings.AgentNotifications.Permission}");
