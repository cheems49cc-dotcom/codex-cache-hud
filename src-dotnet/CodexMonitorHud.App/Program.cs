...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        try
        {
            ...
            _ = NativeMethods.SetCurrentProcessExplicitAppUserModelID("CodexCacheHUD.Desktop");
        }
    ...
    private static int RunSelfTest(HudPaths paths)
    {
        ...
        Console.WriteLine(JsonSerializer.Serialize(new
        {
            ...
            usage_tasks = usageStates.Count,
            usage_subagent_tasks = usageStates.Count(item => item.IsInternalSession),
            runtime_active_tasks = visible.Count(item => item.RuntimeActivityAt != DateTimeOffset.MinValue),
            history_count = state?.CallHistory.Count ?? 0,
            cache_alert = state?.AlertState.ToString() ?? string.Empty,
            latest_call = latestCall is null ? null : new
            {
                timestamp = latestCall.Timestamp.ToString("O"),
                input = latestCall.Input,
                cached = latestCall.Cached,
                cache_write = latestCall.CacheWrite,
                uncached = latestCall.Uncached,
                output = latestCall.Output,
                cache_hit_percent = latestCall.CacheHitPercent,
                context_tokens = latestCall.ContextTokens,
                context_window = latestCall.ContextWindow,
                context_percent = latestCall.ContextPercent,
                alert = latestCall.AlertState.ToString()
            },
